...
##############################################################################
class MsgBufferStorage:
	"""This stores the current alarm, alarm history, and event message serial
	numbers. These are stored separately so that we can reinitialise the 
	HMI message system without losing the current count values. 
	"""
	...
	def __init__(self):
		...
		# This are buffers to keep track of events and alarm history.
		self.EventBuffer = []
		self.AlarmHistory = []

	########################################################
	def SetMsgSerialNumbers(self, eventserial, alarmhistserial):
		"""Reset the serial numbers to the specified values. This can be used 
		to synchronise the alarm system to a data base. If used, it should be 
		called on system initialisation. The serial numbers should be set to 
		a value greater or equal to that what is currently in the database, to 
		avoid having duplicate values.
		Parameters: eventserial (integer) = The serial number to initialise
			the event tracking system to.
			alarmhistserial (integer) = The serial number to initialise the 
				alarm history tracking systems to. 
		"""
		self._EventSerial = eventserial
		self._AlarmHistorySerial = alarmhistserial

	########################################################
	def SetMsgBuffers(self, eventbuff, alarmhistbuff):
		"""Initialise the event and alarm history buffers with stored data.
		Parameters: eventbuff (list) = A list of event messages.
			alarmhistbuff (list) = A list of alarm history messages.
		"""
		self.EventBuffer = eventbuff
		self.AlarmHistory = alarmhistbuff
		

	########################################################
	def GetEventSerial(self):
		"""Increment the the current event serial number and return it.
		"""
		self._EventSerial += 1
		return self._EventSerial


	########################################################
	def GetAlarmHistSerial(self):
		"""Increment the the current alarm history serial number and return it.
		"""
		self._AlarmHistorySerial += 1
		return self._AlarmHistorySerial


MsgBuffers = MsgBufferStorage()
...
class HMIData:
	...
	def __init__(self, datatable, tags, hmitags, eventconfig, alarmconfig):
		...
		self._MaxEventBuffer = 100
		self._MaxAlarmBuffer = 100

		# This is how many to cut from the buffer at one time.
		self._PurgeEvents = 10
		self._PurgeAlarms = 10
		...
		self._EventStates = {}
		evkeys = self._EventConfig.keys()
		evvalues = [False] * len(evkeys)
		self._EventStates = dict(zip(evkeys, evvalues))
		# This is used to track if this was the first event scan after initialisation.
		self._FirstEvScan = True

		# Get the current state of the data table, if possible. This is
		# necessary to prevent generating spurious events. 
		self.InitMessages()
	...
	def InitMessages(self):
		"""Mark that the event messages have just be initialised. This should
		cause the event logic to re-synchronise the states on the next pass
	...
	########################################################
	def _GetMessages(self, clientresponse, messagebuffer, zoneconfig, tagtype, filterresponse):
		...
		Parameters: 
			...
			filterresponse (boolean) - If true, filter the response according to
				zone and maximum response size. If false, just return an unfiltered
				response. This does not affect the filtering applied to serial numbers.
		...
		# order. Get the messages which meet the client's criteria for zone and
		# serial number.
		# Filter according to client limits.
		if filterresponse:
			responsebuffer = [i for i in messagebuffer 
					if i['serial'] > clientserial and i[tagtype] in messagetags]
			# Limit the number of responses to the client's requested limit.
			if len(responsebuffer) > clientmax:
				responsebuffer = responsebuffer[len(responsebuffer) - clientmax:]
		# Unfiltered by zone or size.
		else:
			responsebuffer = [i for i in messagebuffer if i['serial'] > clientserial]
	...
	def GetEvents(self, clientresponse):
		...
		return self._GetMessages(clientresponse, MsgBuffers.EventBuffer, 
				self._EventZones, 'event', True)


	########################################################
	def GetEventsNoLimits(self, serial):
		"""Check the event buffer for any new events to send. Reply with any 
		message that is more recent than the specified serial number.
		This version does not filter responses by zone nor limit the number
		of responses.
		Parameters: serial (integer): Asks for messages which are newer than
			this serial number.
		Returns: A list of event dictionaries with each dictionary containing, 
			serial number, event tag, timestamp and current state. 
			e.g. {"timestamp": 1223094615.0, "serial": 1223094619, 
			...
			events first, and the newest ones last.
		""" 
		# The values used for max or zones do not matter.
		clientresponse = {'serial' : serial, 'max' : 0, 'zones' : []}
		return self._GetMessages(clientresponse, MsgBuffers.EventBuffer, 
				self._EventZones, 'event', False)
	...
	def UpdateEvents(self):
		...
		# This prevents spurious events from being generated on the first scan.
		if self._FirstEvScan:
			self._EventStates.update(currentevents)
			self._FirstEvScan = False

		# Prepare the data to see if there has been a change.
		newevents = currentevents.items()
		oldevents = self._EventStates.items()
		newevents.sort()
		oldevents.sort()

		# This does set arithmetic to find the changes.
		changes = set(newevents) - set(oldevents)

		# We are only interested in changed events that are now true.
		# Reading the serial number also automatically increments it.
		evupdates = [(x,y) for (x,y) in changes if y]

		# Update the changed events in the message buffer.
		MsgBuffers.EventBuffer.extend(map(lambda (evaddr, evstate) : 
				{'serial' : MsgBuffers.GetEventSerial(),  
		...
		if (len(MsgBuffers.EventBuffer) > self._MaxEventBuffer):
			MsgBuffers.EventBuffer = MsgBuffers.EventBuffer[self._PurgeEvents :]
	...
	def _AddToAlarmHistory(self, alarmtag):
		...
		# Reading the serial number also automatically increments it.
		MsgBuffers.AlarmHistory.append({'serial' : MsgBuffers.GetAlarmHistSerial(),  
				'alarm' : alarmtag, 
				'state' : 'inactive', 
		...
		# need to start discarding old messages even if the client hasn't
		# seen them yet. We don't want to fill up the server with messages.
		if (len(MsgBuffers.AlarmHistory) > self._MaxAlarmBuffer):
			MsgBuffers.AlarmHistory = MsgBuffers.AlarmHistory[self._PurgeAlarms :]
	...
	def GetAlarmHistory(self, clientresponse):
		...
		""" 
		return self._GetMessages(clientresponse, MsgBuffers.AlarmHistory, 
				self._AlarmZones, 'alarm', True)


	########################################################
	def GetAlarmHistoryNoLimits(self, serial):
		"""Check the alarm history buffer for any new alarm history messages 
		to send. Reply with any message that is more recent than the specified 
		serial number. This version does not filter responses by zone nor 
