...
basis and asked to update themselves. "AddToWatchdogList" is the equivalent for
monitoring communications time outs. 
...
which is passed to the dislay item. Valid value are:
...
For "chkalarm" alarms, the HMI display object will be passed the 
following values: 
...
The watchdog list is equivalent to the display list, but is for the 
communications watchdog only. "AddToWatchdogList" takes only one parameter,
which is a reference to the display object. The display functions must accept
0 or 1 as an update parameter. 
...
function HMIClient(hostname, url, port, cid, readlist, alarmzonelist, eventzonelist, ansyncoption)

e.g. 
	var MBHMIProtocol = new HMIClient(window.location.hostname, window.location.pathname, 
			window.location.port, "Some client ID", ReadList, AlarmZones, EventZones, true);
...
Parameters:
	...
	tagtype (string) - The type of tag (read, timestamp, serverid, etc.).

function AddToDisplayList(objref, addrlabel, tagtype)

The tagtype parameters specify the source of the data. The available sources are:

	read - The address label specified as the second parameter.
	timestamp - The message time stamp.
	serverid - The message server id.
	stat - The message stat parameter.
	msgid - The message id.
	alarms - The message alarms.
	alarmhistory - The alarm history buffer.
	events - The events buffer.
	errors - The errors buffer.
	status - The message status buffer.
	watchdog - True if the comms watchdog counter has overflowed. 
	unackalarm - Indicates if any unacknowledged alarms are present. 
		0 = None. 1 = at least one in "ok" state. 2 = at least one 
		in "alarm" state. 2 has priority over 1.
...
==========
Set the limit to be used when monitoring the watchdog limit via
the display list. 
Parameters: limit (integer) - The amount of difference permitted
	between the number of messages sent and received before an 
	error is reported. This is stored for later use. The default
	value is 10. 

function SetCommsWatchDogTimeOut(limit)
...
/* Parameters: 	hostname (string) - Host name.
		url (string) - URL to host.
		port (integer) - port for host.
		cid (string) - Client ID string.
		readlist (array) - List of tags to poll.
...
function HMIClient(hostname, url, port, cid, readlist, alarmzonelist, eventzonelist, ansyncoption) {
	...
	this.URL = url;
	this.Port = port;
	this.host = "http://" + hostname + ":" + port  + url;
	...
	// The default watchdog count limit. 
	// This is compared to CommsSendCounter to check for a time out.
	this._WDLimit = 10;
	...
	// The list of objects that monitor the watchdog status.
	this._WatchdogList = [];
	...
	// Flags whether any alarms are present.
	this._AlarmPresent = 0;


	// ##############################################################
	// Change the read list while running.
	// Parameters: readlist: (array) - An array of strings containing the
	// list of address tags to read from the server. This function can be used
	// to change the current readlist "on the fly", but is not required if your
	// readlist does not need to change dynamically.
	function _SetReadList(readlist) {
		this.ReadList = readlist;
		this.HMIJSONObject["read"] = readlist;	
	}
	this.SetReadList = _SetReadList;
	...
	*/
	...
	// ##################################################################
	/*	This is like WriteImmediate, but takes an additional parameter which
		is a reference to this object. This is intended for use with 
		window.setTimeout, which requires this feature, as it executes 
		functions within the context of the window, rather than the context 
		of the object.
		Parmeters: writetag (string) - A valid writable tag.
		writeval (any type) - A legal value for that tag.
		libobj (object) - A reference to this object (the "this").
	*/
	function _WritePulse(writetag, writeval, libobj) {

 		libobj.HMIJSONObject.write[writetag] = writeval;

		// Now, do an update right away.
		// Query the server for updates.
		libobj.SendRequest();
		// Update the display.
		libobj.UpdateDisplay();
	}
	...
	*/
	...
	/*	This is intended to be a private method.
		Checks to see if at least one alarm is present and stores the 
		result in a global variable for later reference. Higher number
		codes take priority over lower numbered codes.
		0 = No current alarms present.
		1 = At least one unacknowledged alarm in the "ok" state.
		2 = At least one acknowledged alarm is still active ("ackalarm").
		3 = At least one unacknowledged alarm in the "alarm" state.
	*/
	function _CheckAlarms() {
		this._AlarmPresent = 0;	// Reset this before we start.
		for (i in this._Alarms) {
			try { 
				switch(this._Alarms[i]["state"]) {
				/* if ok and no higher priority code was found
				   yet, then record it and keep searching.
				*/
				case "ok" : { if (this._AlarmPresent == 0) {
						this._AlarmPresent = 1;
						}
					break;}
				// if ackalarm, then record it and keep searching.
				case "ackalarm" : this._AlarmPresent = 2; break;
				// if alarm, then we can stop looking and exit.
				case "alarm" : this._AlarmPresent = 3; return;
				}
			}
			catch (e) {
				// Ignore the error here.
	...
	to the display list, there should be no need to call this function again.
		...
		objref (object) - A reference to the svg object.
		addrlabel (string) - The name of the data table address tag.
		tagtype (string) - The type of tag (read, timestamp, serverid, 
			stat, msgid, alarms, alarmhistory, events, errors, 
			status, watchdog, chkalarm).
	*/
	...
	// ##################################################################
	/* 	Add an item to the watchdog list. This is similar to the 
	...
	otherwise. 
		Parameters:
		objref (object) - A reference to the HMI object.
	*/
	function _AddToWatchdogList(objref) {
		this._WatchdogList.push(objref);
	}
	...
	*/
	function _UpdateDisplay() {
		...
		for (i in this._DisplayList) {
			...
			case "chkalarm" : presentstate = this._AlarmPresent; break;
			}

			var objref = displayobject["objref"];
			// If we have an error, just ignore it rather than stopping everything.
			try { objref.UpdateScreen(presentstate); }
			catch (e) {  }
		}
	...
	*/
	function _SendRequest() {
		...
		// Check if the comms counter has exceeded the limit.
		if (this.CommsSendCounter > this._WDLimit) {
			var wdresult = 1;
		} else {
			var wdresult = 0;
		}
		// This goes through the list of registered watchdog objects and 
		// updates the current status.
		for (i in this._WatchdogList) {
			this._WatchdogList[i].UpdateScreen(wdresult);
			// var displayobject = this._WatchdogList[i];
			// displayobject.UpdateScreen(wdresult);
		}
	...
	*/
	...
	/* 	Set the limit to be used when checking the communications
	...
	*/
	function _SetCommsWatchDogTimeOut(limit) {
		 this._WDLimit = limit;
	}
	...
	this._CheckAlarms = _CheckAlarms;

	this.AddToDisplayList = _AddToDisplayList;
	this.AddToWatchdogList = _AddToWatchdogList;
	...
	this.SetCommsWatchDogTimeOut = _SetCommsWatchDogTimeOut;
