...
The data table can be initialised with three optional parameters, "unified", 
"overlay", and "expandedregs". 
If unified is True, the Coils are mapped over the Discrete Inputs and they 
form a single list. Writing to a coil will affect the correstponding Discrete 
Input. The same is true for Holding Registers and Input Registers. This is 
refered to as a "unified" data table. 
...
The "expandedregs" parameter determines the maximum holding registers address.
If "expandedregs" is False, the maximum address is 65 535. If "expandedregs" 
is True, the maximum address is 1 048 575. 

The default for all three parameters is False. 
...
class ModbusMemTable:
	...
	########################################################
	def __init__(self, unified = False, overlay = False, expandedregs = False):
		...
		expandedregs (boolean) = If True, then the maximum holding register 
			address is 1 048 575, else the maximum holding register address is
			65,535
		"""
