...
##############################################################################
# Project: 	HMIServer
# Module: 	hmiserversbc.py
# Purpose: 	Web server for HMIServer HMI protocol and SBus.
# Language:	Python 2.5
# Date:		20-Mar-2008.
# Ver:		03-Dec-2010.
# Author:	M. Griffin.
...
as SBus Ethernet. Requests from a web browser are translated into the field
...
-r Port number of remote server. The default is 5050.
-u station address of remote server. The default is 1. (This is SBus specific).
-t Timeout for communications. The default is 5.0 seconds.
-e hElp. (this screen).

Example: "python hmiserversbc.py -p 8080".
...
c:\python25\python hmiserversbc.py -p 8082 -h localhost -r 5050 -u 1 -t 5.0 
...
from mbhmi import HMISBusDataTable as HMIDataTable
import HMIServerCommon
import StatusReporter

# These are for the SBus protocol.
import SBusClient as DataTableClient
...
CmdOpts = HMIServerCommon.GetOptionsClient(5050, _HelpStr)
...
rhost, rport, rtimeout, rstnaddr = CmdOpts.GetRemoteParams()
MBClient = DataTableClient.DataTableAccess(rhost, rport, rtimeout, rstnaddr)
...
StatusReporter.Report.SetCommandParams(CmdOpts.GetPort(), rhost, rport, rtimeout, rstnaddr)
