...
class ModbusMemTable:
	...
	def __init__(self, unified = False, overlay = False):
		...
		self._MaxMem = 65535			# Maximum Modbus address.

		# Initialise the Coils list. 
		self._Coils = [False] * (self._MaxMem + 1)
		...
		if not unified:
			self._DiscInputs = [False] * (self._MaxMem + 1)
		else:
			self._DiscInputs = self._Coils


		# Initialise the Holding Registers list. 
		self._HoldingRegs = [0] * (self._MaxMem + 1)
		...
		# table are mapped over each other.
		if not unified:
			self._InputRegs = [0] * (self._MaxMem + 1)
		else:
	...
	def _GetHoldingRegisters(self, addr, qty):
		...
		Returns a packed binary string.
		"""
		return ModbusDataStrLib.signedintlist2bin(self._HoldingRegs[addr : addr + qty])
	...
	def _SetHoldingRegisters(self, addr, qty, data):
		...
		data (packed binary string) - Data.
		"""
		self._HoldingRegs[addr : addr + qty] = ModbusDataStrLib.signedbin2intlist(data)[:qty]
	...
	def GetMaxAddresses(self):
		...
		"""
		return self._MaxMem, self._MaxMem, self._MaxMem, self._MaxMem
